        ...
        <div className="border-b-4 border-[#0eabeb] rounded-b-[40px]">
          <header className="bg-[#0eabeb] text-white p-3 px-5 rounded-t-lg flex justify-between items-center mb-2 rounded-b-[40px]">
            ...
            <div className="invoice-title text-right">
              ...
              <p className="text-sm font-normal m-0">Invoice No: {formData.billNumber}</p>
            </div>
          </header>
        </div>
        <main className="p-5">
          <div className="flex justify-between mb-8">
            ...
            <section className="invoice-details text-right">
              ...
              <p className="text-sm font-medium text-gray-500">
                ...
                <span className="text-[#0eabeb] pt-2 block">$ {formData.totalAmount?.toFixed(2)}</span>
              </p>
            </section>
          </div>
          ...
          <table className="w-full border-collapse mb-8">
            ...
            <tbody>
              <tr className="bg-white text-sm text-[#4F4F4F] font-medium">
                ...
                <td className="pt-5 px-2 pb-2">$ {formData.amount?.toFixed(2)}</td>
                <td className="pt-5 px-2 pb-2">$ {formData.amount?.toFixed(2)}</td>
              </tr>
              {formData.diseaseName && (
                <tr className="bg-white text-sm text-[#4F4F4F] font-medium">
                  <td className="p-2">Disease Treatment: {formData.diseaseName}</td>
                  <td className="p-2">-</td>
                  <td className="p-2">-</td>
                  <td className="p-2">-</td>
                </tr>
          ...
          <section className="totals text-right">
            <p className="text-[13px] font-medium">
              ...
              <span className="text-gray-400">$ {formData.amount?.toFixed(2)}</span>
            </p>
            <p className="text-[13px] font-medium py-1">
              <strong className="mr-14 text-[#030229]">Tax ({formData.tax}%):</strong>{" "}
            ...
            <p className="text-[13px] font-medium text-[#0eabeb]">
              ...
              <span className="text-[#0eabeb]">$ {formData.totalAmount?.toFixed(2)}</span>
            </p>
          </section>
        </main>
        ...
        <footer className="p-3 px-5 border-t border-gray-300 flex justify-between">
          <section className="terms w-2/3">
            ...
            <p className="text-[14px] font-normal text-gray-400">
              Please pay the amount within the due date. For any queries regarding the bill, 
